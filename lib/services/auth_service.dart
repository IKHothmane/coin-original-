...
class AuthService {
  ...
  Future<UserModel?> signInWithGoogle() async {
    try {
      ...
      return _upsertSocialUser(firebaseUser);
    } on FirebaseAuthException catch (e) {
      await _debugReportGoogleAuth(
        'B',
  ...
  Future<UserModel?> signInWithApple() async {
    try {
      ...
      if (idToken == null) {
        throw 'Connexion Apple annulee ou token manquant';
      }
      ...
      final firebaseUser = userCredential.user;
      if (firebaseUser == null) {
        throw 'Echec de la connexion Firebase avec Apple';
      }

      final displayName = _resolveAppleDisplayName(appleCredential, firebaseUser);
      if (displayName != null &&
      ...
      return _upsertSocialUser(
        firebaseUser,
        preferredName: displayName,
      );
    } on SignInWithAppleAuthorizationException catch (e) {
      if (e.code == AuthorizationErrorCode.canceled) {
        throw 'Connexion Apple annulee par l\'utilisateur';
      }
      throw 'Connexion Apple echouee';
    } on FirebaseAuthException catch (e) {
      throw _handleAuthError(e);
    }
  }

  Future<UserModel> _upsertSocialUser(
    User firebaseUser, {
  ...
  String? _resolveAppleDisplayName(
    ...
    if (given != null && given.isNotEmpty) {
      return family != null && family.isNotEmpty ? '$given $family' : given;
    }
  ...
  Future<void> deleteAccount({String? password}) async {
    ...
    if (user == null) {
      throw 'Aucun utilisateur connecte';
    }
  ...
  Future<void> _reauthenticateCurrentUser(
    ...
    if (providerIds.contains('password')) {
      ...
      if (password == null || password.trim().isEmpty) {
        throw 'REAUTH_PASSWORD_REQUIRED';
      }
      if (email == null || email.isEmpty) {
        throw 'Email introuvable pour la reauthentification';
      }
    ...
    if (providerIds.contains('google.com')) {
      final googleUser = await _googleSignIn.signIn();
      if (googleUser == null) {
        throw 'Connexion Google requise pour supprimer le compte';
      }
      final googleAuth = await googleUser.authentication;
      await user.reauthenticateWithCredential(
    ...
    if (providerIds.contains('apple.com')) {
      ...
      final idToken = appleCredential.identityToken;
      if (idToken == null) {
        throw 'Connexion Apple requise pour supprimer le compte';
      }
      await user.reauthenticateWithCredential(
    ...
    throw 'Methode de connexion non prise en charge pour la suppression';
  }

  Future<void> _deleteUserFirestoreData(String uid) async {
    final favoritesSnapshot =
        await _usersCollection.doc(uid).collection('favorites').get();
